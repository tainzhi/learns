...
static void *mbitstr_open(void *buf, int len) {
  MBITSTR *context = (MBITSTR *)calloc(1, sizeof(MBITSTR));
  if (!context)
    return NULL;
  context->type = BITSTR_MEM;
  context->membuf = (uint8_t *)buf;
  context->memlen = len;
  return context;
}
...
static void *fbitstr_open(char *file, char *mode) {
  FBITSTR *context = (FBITSTR *)calloc(1, sizeof(FBITSTR));
  if (!context)
    return NULL;
...
int bitstr_close(void *stream) {
  ...
  switch (type) {
  case BITSTR_MEM:
    return mbitstr_close(stream);
  case BITSTR_FILE:
    return fbitstr_close(stream);
  }
  return EOF;
}
...
int bitstr_getc(void *stream) {
  ...
  switch (type) {
  case BITSTR_MEM:
    return mbitstr_getc(stream);
  case BITSTR_FILE:
    return fbitstr_getc(stream);
  }
  return EOF;
}
...
int bitstr_putc(int c, void *stream) {
  ...
  switch (type) {
  case BITSTR_MEM:
    return mbitstr_putc(c, stream);
  case BITSTR_FILE:
    return fbitstr_putc(c, stream);
  }
  return EOF;
}
...
int bitstr_seek(void *stream, long offset, int origin) {
  ...
  switch (type) {
  case BITSTR_MEM:
    return mbitstr_seek(stream, offset, origin);
  case BITSTR_FILE:
    return fbitstr_seek(stream, offset, origin);
  }
  return EOF;
}
...
long bitstr_tell(void *stream) {
  ...
  switch (type) {
  case BITSTR_MEM:
    return mbitstr_tell(stream);
  case BITSTR_FILE:
    return fbitstr_tell(stream);
  }
  return EOF;
}
...
#if USE_JPEG_BITSTR
    if (context->bitbuf == 0xff) {
      if (EOF == bitstr_putc(0x00, stream))
        return EOF;
    }
#endif
    context->bitbuf = 0;
...
int bitstr_flush(void *stream, int flag) {
  ...
  // output
  bitstr_put_bits(stream, flag ? -1 : 0,
                  context->bitnum ? 8 - context->bitnum : 0);

  // flush
  switch (context->type) {
  case BITSTR_MEM:
    return mbitstr_flush(stream);
  case BITSTR_FILE:
    return fbitstr_flush(stream);
  }
  return EOF;
}