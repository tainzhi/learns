...
void zigzag_encode(int data[64], const int table[64]) {
    int temp[64];
    for (int i = 0; i < 64; i++) temp[i] = data[table[i]];
    memcpy(data, temp, 64 * sizeof(int));
}

void zigzag_decode(int data[64], const int table[64]) {
    int temp[64];
    for (int i = 0; i < 64; i++) temp[table[i]] = data[i];
    memcpy(data, temp, 64 * sizeof(int));
}
...
void dc_ac_huffman_encode(const int data[64], int * dc, const HUFCODEITEM * dc_huffman_code_tree, const HUFCODEITEM * ac_huffman_code_tree,void * bs)
{
    ...
    *dc = data[0];
    ...
    bitstr_put_bits(bs, dc_huffman_code_tree[size].code, dc_huffman_code_tree[size].depth);
    ...
    if (data[63] == 0) {
        rlelist[eob].run_length = 0;
        rlelist[eob].size = 0;
        rlelist[eob].code = 0;
        j = eob + 1;
    }
    for (i = 0; i < j; i++) {
        uint8_t temp = (rlelist[i].run_length << 4) | (rlelist[i].size << 0);
        bitstr_put_bits(bs, ac_huffman_code_tree[temp].code, ac_huffman_code_tree[temp].depth);
        bitstr_put_bits(bs, rlelist[i].code, rlelist[i].size);
    }
}